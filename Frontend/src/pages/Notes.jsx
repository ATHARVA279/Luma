...
export default function Notes() {
  ...
  const generateNotes = async (topicText = topic) => {
    ...
    try {
      ...
      if (
        errorDetail &&
        (errorDetail.includes("429") || errorDetail.includes("quota"))
      ) {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-zinc-400">
            Generate comprehensive notes, flashcards, and mind maps from your
            content.
          </p>
        </div>
        ...
        {loading && (
          <div className="text-center py-20">
            ...
            <p className="text-zinc-400 mt-4 text-sm animate-pulse">
              Analyzing content & generating notes...
            </p>
          </div>
        )}
                ...
                <div className="space-y-3">
                  {notes.definitions &&
                    (Array.isArray(notes.definitions)
                      ? notes.definitions
                      : Object.entries(notes.definitions)
                    ).map((item, i) => {
                      const term = Array.isArray(notes.definitions)
                        ? item.term || item.word
                        : item[0];
                      const def = Array.isArray(notes.definitions)
                      ...
                      return (
                        ...
                        >
                          <span className="font-bold text-blue-300 text-sm block mb-1">
                            {term}
                          </span>
                ...
                <div className="max-w-2xl mx-auto">
                  <div className="aspect-[16/9] mb-8 relative perspective-1000">
                    ...
                    >
                      {/* Front */}
                      <div
                        className={`absolute inset-0 backface-hidden bg-zinc-800 rounded-2xl p-8 flex flex-col items-center justify-center text-center border border-zinc-700 shadow-xl ${showAnswer ? "invisible" : "visible"}`}
                      >
              ...
              <Card className="p-6">
                ...
                <div className="flex flex-col items-center">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
                    {notes.mind_map.branches &&
                      notes.mind_map.branches.map((branch, i) => (
                        <div
                          key={i}
                          className="p-4 rounded-xl bg-zinc-900/50 border border-zinc-800 hover:border-pink-500/30 transition-colors text-center"
                        >
                          <h3 className="font-bold text-pink-300 mb-2 text-sm">
                            {branch.title}
                          </h3>
